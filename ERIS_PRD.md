...
covering sales, revenue trends, top customers, low stock, dead stock, month-over-month comparison,
underperforming outlets, slowest-moving inventory, supplier debt, and average order value trends.
...
|---|---|---|---|
| 8.1 | Auth consolidation / stability | ✅ | `auth.py` + `pos_auth.py`, single flow, no duplicate legacy auth router registered. Quick Demo Access users correctly seeded with valid password hashes. JWT `sub` claim correctly resolved to `username` (not `email`) in `api/deps.py` `get_current_user()` — fixed 2026-09-07. |
...
| 8.4c | AI Assistant — no fabricated fallback numbers | ✅ | Audited `ai_service.py` end-to-end: 0 hardcoded/fabricated currency/percentage/metric literals in response paths. Re-executed `ai_assistant_evaluation.ipynb` with 11 templates, achieving 100.0% structured grounding accuracy against live DB. |
...
| 2026-09-07 (API Pagination Standardization & Inventory Pagination) | Standardized API pagination across `inventory.py`, `suppliers.py`, and `employees.py` to match `sales.py` and `customers.py`. Accepted both `per_page` and `limit` with `page` query parameters. Returned standardized `{items, total, page, per_page, total_pages}` envelope while preserving unpaginated raw list return when pagination parameters are omitted for backward compatibility. Corrected underlying schema columns in `inventory.py` (`Product.sku`, `Product.selling_price`, `Product.reorder_level`), `supplier_service.py` (`Supplier` search & serialization, `PurchaseOrder` outlet guard), and `employee_models.py`/`employee_service.py` (`Employee.outlet_id`, `first_name` ordering). Added frontend table pagination controls and page indicator to `frontend/src/pages/Inventory.jsx`. Made query hooks resilient in `Suppliers.jsx`, `Employees.jsx`, and `Forecasts.jsx`. Verified all endpoints against live database with 0 dangling imports in live graph. |
