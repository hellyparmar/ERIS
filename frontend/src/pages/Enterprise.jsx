// Value helpers
const safe = (v, fallback = '—') => (v == null || isNaN(v) || v === '') ? fallback : v;
const fmtINR = (v) => v == null || isNaN(v) ? '—' : '₹' + Number(v).toLocaleString('en-IN');
const fmtPct = (v) => v == null || isNaN(v) ? '—' : `${v > 0 ? '+' : ''}${v}%`;
/**
                            ...
                            >
                                <GlassCard className="h-full group hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] transition-all cursor-pointer overflow-hidden p-0">
                                    <div className="h-32 w-full relative">
                                        ...
                                        <div className={`absolute top-4 right-4 px-2 py-1 rounded text-xs font-bold ${(!store.status || store.status === 'active') ? 'bg-green-500 text-white box-shadow-green' :
                                            store.status === 'excellent' ? 'bg-green-500 text-white box-shadow-green' :
                                            store.status === 'good' ? 'bg-blue-500 text-white box-shadow-blue' :
                                            'bg-red-500 text-white box-shadow-red'
                                            }`}>
                                            {store.status ? store.status.toUpperCase().replace('_', ' ') : 'ACTIVE'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-5 space-y-4">
                                        <div className="flex justify-between items-end">
                                            <div>
                                                ...
                                                <p className="text-2xl font-bold text-white shadow-sm">{fmtINR(store.revenue)}</p>
                                            </div>
                                        ...
                                        <div>
                                            <div className="flex justify-between text-xs text-gray-200 font-medium mb-1">
                                                ...
                                                <span>{fmtPct(Math.round((store.revenue / store.target) * 100))}</span>
                                            </div>
