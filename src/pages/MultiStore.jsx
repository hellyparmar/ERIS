...
import {
    Store, MapPin, TrendingUp, TrendingDown, Package, ArrowRightLeft,
    BarChart3, Users, DollarSign, Target, AlertTriangle, Star,
                ...
                <GlassCard variant="gradient" className="p-6 animate-slide-up stagger-3">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className={`text-3xl font-bold ${networkMetrics.avgGrowth >= 0 ? 'text-success' : 'text-danger'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    {regionalPerformance.map((region, idx) => (
                        ...
                        >
                            ...
                            <p className="text-2xl font-bold bg-gradient-to-r from-primary to-purple bg-clip-text text-transparent">
                                {formatCurrency(region.totalRevenue)}
                            </p>
                            <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground">
                                <span>{region.storeCount} stores</span>
                                <span className={region.avgGrowth >= 0 ? 'text-success' : 'text-danger'}>
                            ...
                            <div key={store.id} className="flex items-center justify-between p-3 rounded-lg bg-success/5 border border-success/20">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">{store.location.city}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-success">{formatCurrency(store.performance.monthlyRevenue)}</p>
                                    <p className="text-xs text-muted-foreground">{store.metrics.salesGrowth >= 0 ? '+' : ''}{store.metrics.salesGrowth}%</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-3">
                        {storesNeedingAttention.slice(0, 5).map((store) => (
                            <div key={store.id} className="flex items-center justify-between p-3 rounded-lg bg-warning/5 border border-warning/20">
                                ...
                                <div className="text-right">
                                    <p className="font-bold text-warning">{formatCurrency(store.performance.monthlyRevenue)}</p>
                                    <p className="text-xs text-danger">{store.metrics.salesGrowth}%</p>
                                </div>
                            </div>
                        ))}
                    </div>
                            ...
                            {filteredStores.map((store, idx) => (
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4 text-center">
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${store.type === 'Flagship' ? 'bg-yellow-500/20 text-yellow-500' :
                                            store.type === 'Standard' ? 'bg-blue-500/20 text-blue-500' :
                                                store.type === 'Express' ? 'bg-green-500/20 text-green-500' :
                                    ...
                                    <td className="py-3 px-4 text-right">
                                        <p className="font-bold bg-gradient-to-r from-primary to-purple bg-clip-text text-transparent">
                                            {formatCurrency(store.performance.monthlyRevenue)}
                                        </p>
                                    </td>
                                    <td className="py-3 px-4 text-right">
                                        <span className={`font-semibold ${store.metrics.salesGrowth >= 0 ? 'text-success' : 'text-danger'}`}>
