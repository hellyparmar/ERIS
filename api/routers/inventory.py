...
async def get_inventory_list(
    ...
    base_query = """
        SELECT 
            i.id, i.product_id, i.sku, i.name, p.category, i.current_stock,
            i.reorder_point, i.max_stock, p.selling_price, p.cost_price,
            i.stock_status, i.warehouse_location, i.last_restock_date
        FROM inventory i
    ...
    if search:
        where_clauses.append("(p.name ILIKE :search OR i.sku ILIKE :search)")
    ...
    if stock_status:
        where_clauses.append("i.stock_status = :stock_status")
