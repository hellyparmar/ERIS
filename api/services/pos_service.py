...
from api.db.models import Sale, SaleItem, Inventory, Customer, Product
...
def complete_sale(cart: dict, payment: dict, cashier_id: int, db: Session) -> dict:
    ...
    try:
        ...
        from api.db.models import PaymentStatus
        ...
        # Step 5: Award loyalty points (if customer)
        if cart.get('customer_id'):
            customer = db.query(Customer).filter_by(id=cart['customer_id']).first()
            if customer:
                points_earned = int(cart['subtotal'] / 10)  # 1 point per ₹10
                customer.loyalty_points = (customer.loyalty_points or 0) + points_earned

        # Step 6: Commit transaction (ACID point)
        db.commit()
        db.refresh(sale)
...
def _trigger_post_sale_events(sale_id: int, product_ids: List[int], customer_id: int, db: Session):
    ...
    # Check customer tier upgrade
    if customer_id:
        customer = db.query(Customer).filter_by(id=customer_id).first()
        if customer:
            pts = customer.loyalty_points or 0
            new_tier = (
                "platinum" if pts > 50000 else
                "gold" if pts > 25000 else
                "silver" if pts > 10000 else
                "member"
            )
