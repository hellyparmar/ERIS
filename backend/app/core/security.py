...
async def get_current_user(
    ...
    if user_id_int:
        result = await session.execute(
            select(User).options(selectinload(User.role)).where(User.id == user_id_int, User.is_active == True)
        )
    elif email:
        result = await session.execute(
            select(User).options(selectinload(User.role)).where(User.email == email, User.is_active == True)
        )
    else:
