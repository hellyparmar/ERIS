...
def _check_dowhy():
    try:
        ...
        logger.warning(f"DoWhy unavailable ({e}). Causal analysis will be disabled.")
        return False

DOWHY_AVAILABLE = _check_dowhy()
...
class CausalAnalysisService:
    ...
    def analyze_sales_drivers(self, outlet_id: str, days: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            from app.models.external_factors_models import ExternalFactor as SalesFactors
    ...
    def get_anomaly_explanation(
        ...
        try:
            ...
            actual_sales = (
                ...
                .filter(
                    Sale.outlet_id == outlet_id,
                    func.date(Sale.sale_date) == anomaly_datetime,
                )
                .scalar()
            )
