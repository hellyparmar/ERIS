...
def smtp_circuit_breaker(func: Callable) -> Callable:
    """
    Circuit breaker for SMTP email service
    
    - Opens circuit after 5 consecutive failures
    - Recovers after 60 seconds
    - Retries up to 3 times with exponential backoff
    """
    @circuit(failure_threshold=5, recovery_timeout=60, name="smtp")
    @retry(
        stop=stop_after_attempt(3),
        wait=wait_exponential(multiplier=1, min=2, max=10),
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(f"SMTP service error: {e}")
            raise
...
class FallbackHandler:
    ...
    def whatsapp_fallback(
        ...
        **kwargs
        ...
        """
        logger.warning(f"WhatsApp circuit open, attempting fallback for {message_type}")
        ...
        if customer_email:
            ...
            try:
                ...
                if message_type == "invoice_receipt":
                    return email_service.send_invoice_receipt(
        ...
        # Last resort: Queue for manual follow-up
        return {
            "status": "circuit_open",
            "message": "WhatsApp service temporarily unavailable",
            "action": "queued_for_retry",
            "fallback": "email" if customer_email else "manual"
        }
    
    @staticmethod
    def email_fallback(message_type: str, **kwargs) -> dict:
        """
        ...
        - Log for manual follow-up
        """
        logger.warning(f"Email circuit open for {message_type}")
...
def send_receipt_with_fallback(phone, email, message):
    try:
        return _send_whatsapp_message(phone, message)
    except Exception as e:
